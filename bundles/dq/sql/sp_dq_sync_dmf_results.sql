...
BEGIN
    ...
    BEGIN
        ...
        USING (
            ...
            FROM TABLE(SNOWFLAKE.LOCAL.DATA_QUALITY.DATA_QUALITY_MONITORING_RESULTS(
                REF_ENTITY_DOMAIN => 'TABLE',
                REF_ENTITY_NAME => '${DATABASE}.*.*'
            )) dmr
            WHERE dmr.MEASUREMENT_TIME >= DATEADD(HOUR, -24, CURRENT_TIMESTAMP())
              AND (
                  dmr.TABLE_SCHEMA = 'DQ'
    ...
    EXCEPTION
        WHEN OTHER THEN
            RETURN 'DMF sync skipped: ' || SQLERRM || '. Ensure the executing role has '
                || 'SNOWFLAKE.DATA_QUALITY_MONITORING_RESULTS_READER granted.';
    END;
